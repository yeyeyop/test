...
<%
   // 개행문자 값을 저장한다.
   // 개행문자 값을 newLine 에 저장해서 JSTL 에서 쓴다는 것
...
<c:choose>
   ...
   $("#btnReply").on("click", function() {
      ...
      document.bbsForm.submit();
   		...
   		<c:if test="${boardMe eq 'Y'}"> //현재 하이보드테이블에 등록되어있는 유저아이디와 쿠키아이디가 같을때 Y. 여기서 Y랑 같다면 ~
   ...
   $("#btnDelete").on("click", function(){
      if(confirm("게시물을 삭제 하시겠습니까?") == true)
      {
         $.ajax({
            ...
            url : "/board/delete",
            data : {
               hiBbsSeq : <c:out value="${hiBoard.hiBbsSeq}" /> //hiBbsSeq 얘만 들고 가겠다~
            },
            datatype : "JSON",
